...
class MainActivity : AppCompatActivity(), GoogleApiClient.OnConnectionFailedListener {
    ...
    class MessageViewHolder internal constructor(v: View) : RecyclerView.ViewHolder(v) {
        internal var messageTextView = v.messageTextView
        internal var messageImageView = v.messageImageView
        internal var messengerTextView = v.messengerTextView
        internal var messengerImageView = v.messengerImageView
    }
    ...
    private fun getMessageViewAction(message: Message): Action {
        return Action.Builder(Action.Builder.VIEW_ACTION)
                .setObject(message.name.toString(), MESSAGE_URL + message.id)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent) {
        ...
        if (requestCode == REQUEST_IMAGE) {
            if (resultCode == Activity.RESULT_OK) {
                ...
                val tempMessage = Message(null, username, photoUrl, LOADING_IMAGE_URL)
                firebaseDatabaseReference?.child(MESSAGES_CHILD)
                        ...
                        ?.setValue(tempMessage) { databaseError, databaseReference ->
                            if (databaseError == null) {
                                ...
                                val storageReference = FirebaseStorage.getInstance()
                                        .getReference(firebaseUser?.uid.toString())
                                        .child(key)
